...
public static class MoveColumnRight
{
    internal sealed record MoveColumnRightRequest(Guid ColumnId);

    internal sealed record MoveColumnRightCommand(Guid TeamId, Guid BoardId, Guid ColumnId)
        : IRequest<Result>;

    internal class MoveColumnRightHandler(AppDbContext dbContext)
        : IRequestHandler<MoveColumnRightCommand, Result>
    {
        public async Task<Result> Handle(
            MoveColumnRightCommand request,
        ...
        )
        {
            ...
            try
            {
                ...
                var rightColumn = await dbContext
                    .Columns.Where(c =>
                ...
                {
                    return Result.Fail(
                        new Error("Column is already at the rightmost position").WithMetadata(
                            "HttpCode",
                            400
                        )
                    );
                ...
                // Swap positions
                (rightColumn.Position, currentColumn.Position) = (
    ...
    public class Endpoint : ICarterModule
    {
        ...
        {
            app.MapPatch(
                "/api/teams/{teamId:guid}/boards/{boardId:guid}/columns/{columnId:guid}/right",
                async (Guid teamId, Guid boardId, Guid columnId, IMediator mediator) =>
                {
                    var command = new MoveColumnRightCommand(teamId, boardId, columnId);
